...
import {
  DEFAULT_FAMILLE_PAGE_SIZE, EMPTY_FAMILLE_FORM, FAMILLE_PAGE_SIZE_OPTIONS, FAMILLE_STATUT_LABELS,
} from './familleConstants.js';
import { createFamilleApi, deleteFamilleApi, fetchFamillesApi, updateFamilleApi } from './famillesApi.js';

const EMPTY_PAGINATION = { page: 1, limit: DEFAULT_FAMILLE_PAGE_SIZE, total: 0, totalPages: 0 };

export default function IntendanceFamillesPage() {
  const { hasPermission } = usePermissions();
  const { showSuccess, showError } = useToast();
  const canCreate = hasPermission(PERMISSIONS.INTENDANCE.FAMILLE_CREATE);
  const canUpdate = hasPermission(PERMISSIONS.INTENDANCE.FAMILLE_UPDATE);
  const canDelete = hasPermission(PERMISSIONS.INTENDANCE.FAMILLE_DELETE);
  ...
  const openEdit = (item) => {
    setFormMode('edit');
    setEditing(item);
    setFormValues({ code: item.code ?? '', libelle: item.libelle ?? '', ordre: item.ordre ?? 0, statut: item.statut ?? 'ACTIF' });
    setFormError('');
    setFormOpen(true);
  };
      ...
      <Stack spacing={2.5}>
        <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }} spacing={1.5}>
          <Stack direction="row" spacing={1.5} alignItems="center">
            ...
            <Box>
              <Typography level="h2" sx={{ fontWeight: 700 }}>Familles de bien</Typography>
              <Typography level="body-md" sx={{ color: 'neutral.500' }}>EQ, MO, AC, EL, CL, IT — classes larges du parc.</Typography>
            </Box>
          </Stack>
          {canCreate ? <Button startDecorator={<Plus size={16} />} onClick={openCreate}>Nouvelle famille</Button> : null}
        </Stack>
        <Card variant="outlined" sx={{ borderRadius: 'lg', p: 2 }}>
          <Input startDecorator={<Search size={16} />} placeholder="Rechercher…" value={search} onChange={(e) => setSearch(e.target.value)} />
        </Card>
        ...
        <Sheet variant="outlined" sx={{ borderRadius: 'lg', overflow: 'auto' }}>
          <Table stickyHeader hoverRow sx={{ minWidth: 720 }}>
            <thead>
              <tr>
                <th>Code</th><th>Libellé</th><th>Ordre</th><th>Statut</th>
                ...
                <tr key={item.id}>
                  ...
                  <td>
                    <Stack direction="row" spacing={0.5}>
                      <Chip size="sm" variant="soft" color={item.statut === 'ACTIF' ? 'success' : 'neutral'}>{FAMILLE_STATUT_LABELS[item.statut] ?? item.statut}</Chip>
        ...
        <AppPagination page={pagination.page} totalPages={pagination.totalPages} total={pagination.total} limit={limit} limitOptions={FAMILLE_PAGE_SIZE_OPTIONS} onPageChange={setPage} onLimitChange={setLimit} />
      </Stack>
      <FamilleFormModal open={formOpen} mode={formMode} initialValues={formValues} loading={formLoading} error={formError} onClose={() => setFormOpen(false)} onSubmit={handleSubmit} />
      <ConfirmModal
