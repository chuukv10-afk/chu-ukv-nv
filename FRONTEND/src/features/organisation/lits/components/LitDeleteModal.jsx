...
export default function LitDeleteModal({ open, lit, loading = false, error = '', onClose, onConfirm }) {
  return (
      ...
      <ModalDialog variant="outlined" role="alertdialog" sx={{ borderRadius: 'xl', maxWidth: 420, p: 3, boxShadow: 'lg' }}>
        <Stack spacing={2}>
          <Stack direction="row" spacing={1.5} alignItems="center">
            ...
            <Box>
              <Typography level="title-lg" sx={{ fontWeight: 700 }}>Supprimer ce lit ?</Typography>
              <Typography level="body-sm" sx={{ color: 'neutral.500' }}>{lit ? `${lit.code} — ${lit.numeroLit}` : ''}</Typography>
            </Box>
          </Stack>
          {error ? <Typography level="body-sm" color="danger">{error}</Typography> : (
            <Typography level="body-sm" sx={{ color: 'neutral.600' }}>Le lit ne doit plus être affecté à des visites.</Typography>
          )}
