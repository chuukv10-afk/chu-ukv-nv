...
export default function ServiceDeleteModal({ open, service, loading = false, error = '', onClose, onConfirm }) {
  return (
      ...
      <ModalDialog variant="outlined" role="alertdialog" sx={{ borderRadius: 'xl', maxWidth: 420, p: 3, boxShadow: 'lg' }}>
        <Stack spacing={2}>
          <Stack direction="row" spacing={1.5} alignItems="center">
            ...
            <Box>
              <Typography level="title-lg" sx={{ fontWeight: 700 }}>Supprimer ce service ?</Typography>
              <Typography level="body-sm" sx={{ color: 'neutral.500' }}>{service ? `${service.code} — ${service.libelle}` : ''}</Typography>
            </Box>
          </Stack>
          {error ? <Typography level="body-sm" color="danger">{error}</Typography> : (
            <Typography level="body-sm" sx={{ color: 'neutral.600' }}>Le service ne doit plus être affecté à du personnel.</Typography>
          )}
