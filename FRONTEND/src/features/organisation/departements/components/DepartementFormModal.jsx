...
import { DEPARTEMENT_TYPES } from '../departementConstants.js';

const MODAL_SX = { borderRadius: 'xl', maxWidth: 480, p: 0, overflow: 'hidden', boxShadow: 'lg' };

export default function DepartementFormModal({
      ...
      <ModalDialog variant="outlined" sx={MODAL_SX}>
        <Box sx={{ px: 3, py: 2.5, borderBottom: '1px solid', borderColor: 'divider' }}>
          <Stack direction="row" spacing={1.5} alignItems="center">
            ...
            <Box>
              <Typography level="title-lg" sx={{ fontWeight: 700 }}>{isEdit ? 'Modifier le département' : 'Nouveau département'}</Typography>
            </Box>
          </Stack>
        </Box>
        <Box component="form" onSubmit={handleSubmit} sx={{ p: 3 }}>
          <Stack spacing={2}>
            ...
            <FormControl required>
              ...
              <Input value={form.code} onChange={(e) => handleChange('code', e.target.value.toUpperCase())} disabled={loading} slotProps={{ input: { maxLength: 8 } }} />
            </FormControl>
            <FormControl required>
            ...
            <FormControl required>
              ...
              <Select value={form.type} onChange={(_, v) => handleChange('type', v)} disabled={loading}>
