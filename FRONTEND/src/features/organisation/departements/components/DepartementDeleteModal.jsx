...
export default function DepartementDeleteModal({ open, departement, loading = false, error = '', onClose, onConfirm }) {
  return (
      ...
      <ModalDialog variant="outlined" role="alertdialog" sx={{ borderRadius: 'xl', maxWidth: 420, p: 3, boxShadow: 'lg' }}>
        <Stack spacing={2}>
          <Stack direction="row" spacing={1.5} alignItems="center">
            ...
            <Box>
              <Typography level="title-lg" sx={{ fontWeight: 700 }}>Supprimer ce département ?</Typography>
              <Typography level="body-sm" sx={{ color: 'neutral.500' }}>{departement ? `${departement.code} — ${departement.libelle}` : ''}</Typography>
            </Box>
          </Stack>
          {error ? <Typography level="body-sm" color="danger">{error}</Typography> : (
            <Typography level="body-sm" sx={{ color: 'neutral.600' }}>Le département ne doit plus contenir de services.</Typography>
          )}
