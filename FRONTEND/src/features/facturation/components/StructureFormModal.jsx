...
import { STRUCTURE_STATUTS, STRUCTURE_TYPES } from '../facturationConstants.js';
      ...
      <ModalDialog variant="outlined" sx={MODAL_SX}>
        <Box sx={{ px: 3, py: 2.5, borderBottom: '1px solid', borderColor: 'divider', flexShrink: 0 }}>
          <Stack direction="row" spacing={1.5} alignItems="center">
            ...
            <Typography level="title-lg" sx={{ fontWeight: 700 }}>
              {isEdit ? 'Modifier la structure' : 'Nouvelle structure'}
            </Typography>
          </Stack>
        </Box>
        <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', minHeight: 0 }}>
            ...
            <Stack spacing={2}>
              ...
              <FormControl required>
                ...
                <Input
                  ...
                  placeholder="Ex. Mutuelle SOS"
                />
              </FormControl>

              <FormControl required>
                <FormLabel>Type</FormLabel>
                <Select value={form.type} onChange={(_, value) => handleChange('type', value ?? 'MUTUELLE')} disabled={loading}>
          ...
          <Stack direction="row" spacing={1.5} justifyContent="flex-end" sx={{ px: 3, py: 2, borderTop: '1px solid', borderColor: 'divider', flexShrink: 0 }}>
            ...
            <Button type="submit" loading={loading}>{isEdit ? 'Enregistrer' : 'Créer la structure'}</Button>
          </Stack>
        </Box>
      </ModalDialog>
    </Modal>
