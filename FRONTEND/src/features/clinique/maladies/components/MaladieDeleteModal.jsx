...
export default function MaladieDeleteModal({ open, maladie, loading = false, error = '', onClose, onConfirm }) {
  return (
      ...
      <ModalDialog variant="outlined" role="alertdialog" sx={{ borderRadius: 'xl', maxWidth: 420, p: 3, boxShadow: 'lg' }}>
        <Stack spacing={2}>
          <Stack direction="row" spacing={1.5} alignItems="center">
            ...
            <Box>
              <Typography level="title-lg" sx={{ fontWeight: 700 }}>Supprimer cette maladie ?</Typography>
              <Typography level="body-sm" sx={{ color: 'neutral.500' }}>{maladie ? `${maladie.codeCim10} — ${maladie.libelle}` : ''}</Typography>
            </Box>
          </Stack>
          {error ? <Typography level="body-sm" color="danger">{error}</Typography> : (
            <Typography level="body-sm" sx={{ color: 'neutral.600' }}>La maladie ne doit plus être utilisée dans des antécédents ou diagnostics.</Typography>
          )}
