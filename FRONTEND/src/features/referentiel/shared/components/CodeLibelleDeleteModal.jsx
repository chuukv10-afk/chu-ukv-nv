...
export default function CodeLibelleDeleteModal({
  open,
  item,
  loading = false,
  error = '',
        ...
        <Stack spacing={2}>
          <Stack direction="row" spacing={1.5} alignItems="center">
            ...
            <Box>
              ...
              <Typography level="body-sm" sx={{ color: 'neutral.500' }}>{item ? `${item.code} — ${item.libelle}` : ''}</Typography>
            </Box>
          </Stack>
          {error ? <Typography level="body-sm" color="danger">{error}</Typography> : (
            <Typography level="body-sm" sx={{ color: 'neutral.600' }}>{hint}</Typography>
          )}
